...
Use the configured `quill` MCP server. Start with `get_quill_status`, then `list_managed_accounts`; choose the target `accountId` before reading that account's `get_profiles` or changing content.
...
4. Use `update_draft`, `retry_failed_post`, `list_drafts`, `discard_draft`, `list_scheduled_posts`, and `cancel_scheduled_post` to manage the queue. Discard/cancel only when explicitly requested.

...
- Highlight match keywords while browsing without saving anything. Use **Start manual collection**, then scroll X yourself; Quill captures only after your scroll events and shows a persistent live **Collected** count on the left side. Stop it any time, or use **Save visible matches** for a one-time capture. Repeats are safely refreshed in Quill, and very large batches are split automatically.

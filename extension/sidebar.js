...
async function startScan() { try { const result = await sendToTab("QUILL_START_SCAN"); $("scanBadge").textContent = result.status === "started" ? "Scanning" : "Already scanning"; setStatus("scanStatus", "Scanning and saving keyword matches…"); } catch (error) { setStatus("scanStatus", error.message); } }

...
export async function registerArticleRoutes(app: FastifyInstance, prisma: PrismaClient) {
  ...
  app.patch("/api/articles/:id", async (request) => {
    ...
    const body = z.object({ title: z.string().trim().min(1).max(400), contentState, coverAssetId: z.string().uuid().nullable().optional() }).parse(request.body);
    return { article: await articles.updateDraft(id, { xAccount, ...body }) };
  });

  app.delete("/api/articles/:id", async (request) => {
    const xAccount = await managedAccountForRequest(prisma, request);
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params);
    return articles.deleteDraft(id, xAccount.id);
  });
